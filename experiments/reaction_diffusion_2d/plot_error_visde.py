...
def main(train_val_test: str = "test"):
    ...
    figrmse, ax = plt.subplots(figsize=(6.3, 3.2))

    ticks = [0, (n_tsteps-1)//4, (n_tsteps-1)//2, (n_tsteps-1)*3//4, -1]
    t_plot = t.cpu().detach().numpy()[0, :]

    ax.plot(t_plot, np.mean(rel_err, axis=0))
    #ax.boxplot(rel_err[:, ticks], positions=t_plot[ticks], showfliers=False)
    ax.set_xticks(t_plot[ticks], [f"{t_plot[tick]:.1f}" for tick in ticks])
    ax.set_xlabel("Time")
    #ax.set_xlim([-0.1, 1.1])
    ax.set_ylabel(r"$\|\bar{u}(t) - u(t)\|/\|u(t)\|$")
    #ax.set_ylim([0, 0.04])
    #ax.set_title(f"Mean error: {np.mean(rmsre)*100.0:.4f}%")
    ax.grid()
    figrmse.tight_layout()
    figrmse.savefig(os.path.join(OUT_DIR, f"{train_val_test}_rmsre_traj_mean_{DATA_FILE}.pdf"), format='pdf')
    figrmse.show()
