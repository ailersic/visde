...
def main():
    ...
    plt.figure()
    plt.plot(np.linspace(1, min(n_snap, dim_x), min(n_snap, dim_x)), torch.cumsum(energy_frac, 0).numpy())
    plt.xscale("log")
    ...
    print(f'{torch.nonzero(torch.cumsum(energy_frac, 0) > 0.99)[0, 0].item() + 1} modes capture 99% of the energy')
