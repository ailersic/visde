...
def main(train_val_test: str = "test"):
    ...
    figrmse, ax = plt.subplots(figsize=(6.3, 3))

    periods = [0, 250, 500, 750, 1000]
    ...
    ax.set_xticks(t_plot[periods], [r"$0$", r"$0.25$", r"$0.5$", r"$0.75$", r"$1$"])
    ax.set_xlabel("Time")
    ax.set_xlim([-0.1, 1.1])
    ...
    figrmse.savefig(os.path.join(OUT_DIR, f"{train_val_test}_{N_TRAIN}_rmsre_traj_mean.pdf"), format='pdf')
    figrmse.show()
